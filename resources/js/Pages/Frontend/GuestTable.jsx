...
export default function GuestTable() {
  ...
  const stats = useMemo(() => {
    ...
    const assigned = tables.reduce((sum, t) => {
      ...
      return (
        sum +
        seats.reduce((seatSum, s) => seatSum + (s.assignment?.guest ? 1 : 0), 0)
      );
    }, 0);
  ...
  const visibleTables = useMemo(() => {
    ...
    return tables.filter((t) => (t.seats || []).some((s) => !s.assignment?.guest));
  ...
  return (
    <div className="min-h-screen  px-4 py-8 bg-sky-100 sm:px-6">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        {flash?.success && (
          <div className="rounded-2xl border border-black/10 bg-white/80 p-3 text-sm">
            {flash.success}
          </div>
        )}
        ...
        <div className="grid gap-3 sm:grid-cols-4">
          ...
          <div className="rounded-2xl border border-black/10 bg-white/80 p-4">
            ...
            <div className="text-2xl font-bold">{stats.blankSeats}</div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-black/10 bg-white/70 p-4 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="text-sm font-semibold">Search guest</div>
                ...
                searchResults.map((g) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-gray-500">
                        {g.tableName} • Seat {g.seatNo}
                      </div>
                    </div>
                    <span className="text-xs font-semibold">
                      {g.checkedIn ? "Present" : "Absent"}
                    </span>
                  </button>
                ))
              )}
